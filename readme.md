...
## 被舍弃的功能
菜单栏“家庭”，课题组成员可以通过菜单栏的家庭按钮进入家庭页（家庭其实就是组织，或者说集体社区），没有家庭的话可以在页面里加入列出的已有家庭，也可以自己创建。加入或创建家庭之后家庭页面会更新，加入后也可以自主退出。家庭不存在负责人。
...
舍弃原因是，感觉这就是个微信，我的网站定位不是即时通讯，实际上，我认为即时通讯严重降低了沟通效率，我很反感。

## 新增功能
实验室位置页面将物品的三个按钮（添加、新建、移除）放在物品统计的下方，此外补充这个位置页面的编辑、删除、返回列表按钮，与物品详情页统一。关于移除物品，不要直接在包含的物品里放勾选框，改成加入物品类似的弹窗，在弹窗里面勾选，然后页面上则不需要展示包含物品的详细列表，实现与物品详情页的一致性。不过物品统计里的信息可以补充更多，比如包含物品里哪个负责人最多，所含物品属于哪些物品类别等等你认为可以加进去的。

用户个人主页里，负责的物品和位置展示五个，剩下的折叠。负责物品里不需要显示位置，物品和位置的显示外观一致。“个人展示/备注”的展示进行适当的UI美化，可以实现自动对“链接”进行特殊展示，让用户可以直接点击跳转，将“#XXX”识别为标签，将@XX识别为提到其他用户等等。

将现在的留言板块进行优化，我认为不需要新建一个数据库表单来专门存储，我认为直接在用户的属性里多加一个类似于“个人展示/备注”的“他人评价/留言”，每个人留言的过程就是在这个里面新增文本内容的过程（即规定一套语法规则后端写入），当然啊，会附带上留言人的个人信息和留言时间，前端自动渲染，也有类似的其他的自动识别功能，比如把”!!”识别为好评，”??”识别为怀疑和差评。所有的留言和评价不支持任何人删除和编辑，任何人只能在里面写入新信息。

将所有“按住 Ctrl/Cmd 或 Shift 可多选；留空表示暂未指定。”的编辑逻辑进行优化，比如活动的物品增减采用新增关注和地点管理存放物品的弹窗那样，简化操作、支持多端编辑的同时还能实现网站的统一。将用户个人主页的“他人评价/留言”板块也搬到活动事项里面，让用户可以实现对活动进行留言讨论和评价等等。